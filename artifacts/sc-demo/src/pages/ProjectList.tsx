...
import {
  ChevronRight, Calendar, DollarSign, User, Briefcase,
  Search, Activity, PauseCircle, CheckCircle2, Clock, FileSpreadsheet
} from "lucide-react";
import Layout from "@/components/Layout";
import { MOCK_PROJECTS, Project } from "@/lib/mockData";
import { Input } from "@/components/ui/input";

const DETAIL_LOAD_MS = 1800;
...
export default function ProjectList() {
  ...
  // Animate the progress bar and flip detailReady when done
  useEffect(() => {
    ...
    const tick = (now: number) => {
      ...
      const pct = Math.min((elapsed / DETAIL_LOAD_MS) * 100, 100);
      setProgress(pct);
                ...
                <div className="p-6 md:p-8 bg-slate-50/50 flex-1 flex flex-col">
                  ...
                  <div className="flex gap-3 mt-auto">
                    <div className="flex-1 relative overflow-hidden rounded-lg">
                      {/* Progress bar fills from left behind the button */}
                      {!detailReady && (
                        <div
                          className="absolute inset-0 bg-primary/20 transition-none rounded-lg"
                      ...
                      >
                        <button
                          className={`relative w-full font-medium py-2.5 px-4 rounded-lg shadow-sm text-sm flex items-center justify-center gap-2 transition-colors border ${
                            detailReady
                    ...
                    <Link href={`/projects/${selectedProject.id}/settings`} className="flex-1">
                      <button
                        className="w-full bg-white hover:bg-slate-50 text-slate-700 border border-slate-200 font-medium py-2.5 px-4 rounded-lg shadow-sm transition-colors text-sm"
